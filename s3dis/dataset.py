...
from backbone.camera_outside import CameraOptions, CameraHelper, make_cam_points, merge_cam_points
...
class S3DIS(Dataset):
    ...
    def get_test_item(self, idx):
        ...
        visible_ds = torch.ones(xyz.shape[0], dtype=torch.bool)
        visible_full = torch.ones(full_xyz.shape[0], dtype=torch.bool)
        kdt = KDTree(xyz.numpy(), visible_ds.numpy())
        _, full_nn = kdt.query(full_xyz.numpy(), visible_full.numpy(), k=1)
        full_nn = torch.as_tensor(full_nn, dtype=torch.long).reshape(-1)
...
def s3dis_test_collate_fn(batch):
    ...
    cam_points_list = list(cam_points_list)
    new_cam_points = merge_cam_points(cam_points_list)
